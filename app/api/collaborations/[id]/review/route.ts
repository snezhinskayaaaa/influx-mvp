...
export async function POST(
  ...
  try {
    ...
    const { success } = await rateLimit(`collab-review:${user.userId}`, 5, 60000)
    if (!success) {
    ...
    if (action === 'accept_invitation' || action === 'decline_invitation') {
      ...
      if (action === 'accept_invitation') {
        // Skip APPLIED — go straight to NEGOTIATING with the proposed price
        // Brand already agreed by inviting, so set brandAgreed: true
